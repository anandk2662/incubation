...
export default function DemoLab() {
  ...
  return (
    <div className="min-h-screen md:h-screen pt-16 md:overflow-hidden bg-bg-primary flex flex-col">
      <div className="flex flex-col md:grid md:grid-cols-[280px_1fr] lg:grid-cols-[280px_1fr_300px] flex-1 min-h-0">
        ...
        <div className="md:hidden flex overflow-x-auto p-4 gap-3 no-scrollbar border-b border-border-primary bg-bg-secondary sticky top-16 z-30">
          {demos.map(demo => (
            <button
              key={demo.id}
        ...
        <aside className="hidden md:flex flex-col border-r border-border-primary overflow-y-auto">
          ...
          <nav className="flex flex-col p-2 gap-1">
            {demos.map(demo => (
              <button
                key={demo.id}
                className={`flex items-center gap-3 px-4 py-3 rounded-sm transition-all text-left ${
                  activeDemoId === demo.id 
                    ? 'bg-bg-elevated' 
                    : 'hover:bg-bg-hover'
                }`}
        ...
        <main className="bg-bg-secondary p-4 sm:p-6 md:overflow-hidden flex flex-col min-w-0 min-h-[400px] md:min-h-0">
          <div className="flex-1 flex flex-col bg-bg-surface border border-border-strong rounded-xl overflow-hidden shadow-2xl">
            <div className="h-12 bg-bg-surface border-b border-border-primary flex items-center justify-between px-4 shrink-0">
            ...
            <div className="flex-1 bg-black overflow-y-auto">
              {activeDemo.type === 'terminal' ? (
                <div className="p-6 font-mono text-xs sm:text-sm leading-relaxed text-text-secondary">
                  ...
                  <div className="inline-block w-2 h-4.5 bg-white ml-0.5 align-middle animate-[blink_1s_step-end_infinite]" />
                </div>
              ) : (
                <div className="h-full flex flex-col items-center justify-center p-8 text-center">
                  <Layout size={48} className="text-text-tertiary mb-6" />
                  <h3 className="text-xl font-bold text-text-primary mb-2">{activeDemo.title} Interface</h3>
                  <p className="text-text-secondary max-w-sm mb-8">This interactive prototype is optimized for a full-screen experience.</p>
                  <Button variant="primary" icon={<Play size={16} />}>Launch Full Prototype</Button>
                </div>
              )}
            </div>
          </div>
        ...
        <aside className="flex flex-col border-t md:border-t-0 md:border-l border-border-primary p-6 md:overflow-y-auto bg-bg-primary md:bg-transparent">
          <div className="mb-8 md:mb-10">
            <h3 className="flex items-center gap-2.5 text-[11px] font-bold uppercase tracking-widest text-text-tertiary mb-4">
              <Info size={16} />
              Information
            </h3>
          ...
          <div className="h-px bg-border-primary mb-8 md:mb-10" />

          <div className="mb-8 md:mb-10">
            <h3 className="text-[11px] font-bold uppercase tracking-widest text-text-tertiary mb-4">Status</h3>
          ...
          <div>
            <h3 className="text-[11px] font-bold uppercase tracking-widest text-text-tertiary mb-4">Project Context</h3>
